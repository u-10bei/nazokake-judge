...
| **BR-U4a-05（プール充足の事前検証・三点セット）** | 投入時に本番セッション構成可能性を事前チェック。次の**すべて**を満たすこと（参加者アクセス前に弾く, BR-05）: <br> ① `総数 ≥ ceil(2 × session_pairs / k)`（ペア構成の最小数） <br> ② **4 層すべて非空** <br> ③ `(総数 − 最大層の件数) × k ≥ ceil(cross_layer_min_ratio × session_pairs)`（層間ペアの供給可能性） <br> 不足時は **error ログ（不足内訳を含む）+ 投入拒否**。 | Q6 / BR-05 / XC-01 |
...
| プール充足不足 | `precheck_errors` に不足内訳・投入拒否（BR-U4a-05） |
