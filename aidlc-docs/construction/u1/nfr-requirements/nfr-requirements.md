...
**非目標（意図的な非要件）**:
- 高可用性・自動フェイルオーバー・DR は設計しない（NFR-06）。データ保全は回答の逐次保存 + 原子的ペア列確定で足りる（NFR-05）。
- **同時セッション開始時の露出スナップショット競合は許容する**（ロック・直列化は設けない）。鮮度ズレは P-1 の累積収束で吸収され、各セッションの `exposure_snapshot` は「実際に参照した値」として保存されるため監査リプレイの完全性は損なわれない（Q8=A）。
