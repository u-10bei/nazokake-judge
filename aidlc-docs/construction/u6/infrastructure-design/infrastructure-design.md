...
- **✅ セット別の確定値**（2026-07-20 確定・**構成可能性を実測検証済み**）:
  ...
  **フォールバック版 n=34 の実測**: 辺数 204（=J 一致）・**次数 gap=0**・**連結成分 1**・**充足ゲート通過**（最大層 edit14 → (34−14)×3=60 ≥ 26）・**層間比率 0.716**（≥0.65）。→ **すべて成立**。
  **n=36 へ寄せない**（[27×8] の均一分割は**美観であって測定上の利得ではない**）。理由は BR-U6-12 参照（自由在庫の温存・層割当の再議論回避）。

- **★ `plan_generate` の入力は「三つ組」でセット単位**（BR-U6-21）: **（プール・期待組成・制約ファイル）**。**制約ファイルもセット別**であり成立版のものは流用不可（pivot 衝突照合の再走・N8 濃縮の消滅・N 系隣接回避の消滅）。**フォールバック版の制約ファイルは n=34 前提で別途作成**（タスク5 側の TODO）。
