...
**✅ 確定（2026-07-20）**: **フォールバック版 = n=34 / J=204 / [26×4, 25×4]**（pro6〈S04・S10・S02・S11・S12・S19〉/ anchor2 / edit14 / ai9 / rule3）。**フォールバック予約は S02・S11・S12・S19 のちょうど 4 件**（38−8+4=34）。**n=36 は根拠のない値**だった。
**構成可能性を実測検証**: 辺数 204（=J 一致）・次数 gap=0・連結成分 1・充足ゲート通過・層間 0.716 → **すべて成立**。
**n=36 へ寄せない判断**: [27×8] の均一分割は**美観であって測定上の利得ではない**（成立版が既に 29/28 混在）。**自由在庫 S08・S14・S15・S17 は「初回 BT で中間帯が不安定だった場合の追補口」として温存確定**——**劣化条件下でさらに予備を減らすのは悪手**。中間帯古典は anchor でも pro でもなく**層割当の再議論を生む**。
**★追加明文化**: **`plan_generate` の入力は（プール・期待組成・制約ファイル）の三つ組でセット単位**。**制約ファイルもセット別**（pivot 衝突照合を n=34 で再走・N8 濃縮は消滅・N 系隣接回避も消滅）→ **フォールバック版の制約ファイルは別途作成**（タスク5 §4 に一行追記を推奨）。
