...
## 生成予定の成果物（Part 2）
- [ ] `construction/u5/functional-design/business-rules.md`（BR-U5-NN）
- [ ] `construction/u5/functional-design/business-logic-model.md`（廃止フローと参照先の分離）
- [ ] `construction/u5/functional-design/domain-entities.md`（`items` スキーマ差分 = migration 0004・`sessions` 差分）
...
**回答後の流れ**: 曖昧点を点検（あれば追加質問）→ Part 2 で business-rules / business-logic-model / domain-entities を生成 → 標準 2 択（Request Changes / Continue → **NFR Requirements〈U5〉**）。回答は本 plan の各 `[Answer]:` 欄へ書き戻す。
