...
- [x] **Step 4 — `constraints`（LC-U6-01）**: 制約ファイル + **期待組成**の読込・検証。**三つ組（プール・期待組成・制約ファイル）はセット単位**。期待組成の不一致で**明示失敗**（BR-U6-22）。
...
- [x] **Step 5 — `placement`（LC-U6-02・★制約付き探索）**: 円周配置を探索。**目的**: ① 禁止ペアを距離 >6（ハード）② 層間比率 ≥0.65 ③ 濃縮対象を近接 ④ 忌避ペアを可能なら遠くに（ソフト）。**実測で 3 目的の同時成立を確認済み**（違反 0 / 層間 0.706 / 濃縮 9-9）。
- [x] **Step 6 — `graph_build`（LC-U6-03）**: `C_n(1..m/2)` を構成。**構成で保証**: 露出 gap=0 / 全体連結 / 同一ペア 0。
- [x] **Step 7 — `partition`（LC-U6-04）**: J 辺を E スロットへ分割（k≤3 を満たす貪欲配分）。
- [x] **Step 8 — `sequencing`（LC-U6-05）**: スロット内のペア列を並べ替え（**隣接回避**）。**練習ペアを全評価者共通で固定記載**（`is_practice=1`）。**出力順がそのまま `pair_index`**。**★追補: 練習ペアはペア列の先頭（`pair_index` の最小側）に置く**（既存 U2 の `derive_phase` が「練習 → 本番」の順で進むため。位置が未規定だと実装依存になる）。
- [x] **Step 9 — `verify`（LC-U6-06）**: **BR-U6-10 の①〜⑥ + PU6-8（禁止辺不在）**を検証し `PlanVerification` を返す。**忌避はレポートのみ**（失敗させない）。
  **★追補（Likert 配線）**: **`likert_targets` が (i) ちょうど 10 件 (ii) すべてプール内に実在 (iii) 重複なし**であることを検証（違反は明示失敗）。
- [x] **Step 10 — `plan_generate` CLI（LC-U6-07）**: **構成 → 検証 → 失敗なら seed を進めて再試行 → 上限で明示失敗**。**明示失敗**: 正則不能（`2J` が `n` で割り切れない）/ **分割総和 ≠ J** / 期待組成不一致 / 再試行上限。**メタ記録**: 初期 `seed` + **成功試行番号** + **内容ハッシュ**。
...
## Part 2 進捗 — ブロック 2（生成器: Step 4〜10）完了（2026-07-20）

`scripts/plan_generate/` を **LC 一対一で 7 ファイル**に分割生成（U4b `bt_aggregate` と同型）。
unit+PBT **76 緑**（回帰なし）。

### 実データ規模での実行結果（成立版 n=38 / E=8 / J=228・attempt=0 で成功）
...
| ④ 同一ペア | **0** ✅ |
| ⑤ 層間比率 | **0.706** ✅ |
| PU6-8 禁止辺の残存 | **0 本**（8 本すべて排除）✅ |
| 濃縮 | **9/9 本** 達成 |
| 練習の位置 | `idx=[0,1,2]`（**先頭**）✅ |
| Likert 固定リスト | **10 件**を meta に格納 ✅ |

**決定論（BR-U6-11）**: 同一 seed で再実行 → **content_hash 完全一致**（`b1320dd50c1a…`）。
**フォールバック版（n=34 / J=204）**: attempt=0 で成功・**gap=0 / 成分 1 / ブロック [1,1] / 層間 0.755**。

### 失敗系（U6-NFR-11）— 5 ケースすべて明示失敗（exit=1）

| ケース | メッセージ |
|---|---|
| **plan_set 取り違え** | 「制約ファイル=primary / 期待組成=fallback（セット別の三つ組ゆえ流用不可）」 |
| J = n·m/2 不整合 | 「n=38 m=12 → 期待 J=228 だが n_pairs=200」 |
| m 奇数（正則不能） | 「m=11 が奇数（巡回グラフで構成できない）」 |
| **★anchor 投入忘れ**（BR-U6-22） | 「プール総数不一致: 実 38 / 期待 36」 |
