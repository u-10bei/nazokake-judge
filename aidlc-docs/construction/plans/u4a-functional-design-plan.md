...
[Answer]: A ＋ 破壊的更新ガードを必須の business rule 化（`pairs`/`judgments` から参照済みの item_id への UPDATE は拒否＝プール凍結の強制, BR-U4a-03）。新規 INSERT は常に可。
...
[Answer]: X（★A を修正）— Item に本文 `body`（必須）を追加し **D1 に格納**、`body_ref` は出自メモに格下げ（任意）。入力形式は A どおり JSON/JSONL。投入 JSON は gitignore（NFR-08 の実装）。**U1 波及**（`schema/models.py` Item 変更・`migrations/0002_item_body.sql`・`list_items` 更新・テスト更新）を U4a スコープに含む。
...
[Answer]: A ＋ 層間実行可能性の式を追加し三点セット化: ① 総数 ≥ ceil(2×session_pairs/k) ② 4 層すべて非空 ③ `(総数 − 最大層件数) × k ≥ ceil(cross_layer_min_ratio × session_pairs)`。不足は error（内訳）+ 投入拒否（後に BR-U4a-05/12 で段階投入対応に再整理）。
...
[Answer]: A ＋ 衝突処理は「(i) 発行前に既存トークン集合を読み衝突を事前排除 → (ii) batch 投入 → (iii) 失敗したら全体を再生成しリトライ」の順。
