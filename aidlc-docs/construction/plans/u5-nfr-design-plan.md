...
## 生成予定の成果物（Part 2）→ 生成済み（2026-07-17, 全 5 問 A）
- [x] `construction/u5/nfr-design/nfr-design-patterns.md`（DP-U5-NN + 非採用部品表）
- [x] `construction/u5/nfr-design/logical-components.md`（LC-U5-NN + 依存方向）
...
**回答サマリ**: 全 5 問 A。Q1=`Session.likert_targets` をモデルに載せ **`save_pair_sequence` の同一 batch で原子保存**（`exposure_snapshot` の前例に従う・中間状態を原理的に排除・PBT-02/XC-02 に自然に載る）。Q2=`list_items()` 凍結 + `list_active_items()` 新設・呼び出し先を LC で固定。Q3=`get_likert_targets` はサービス層（`session.py`）・domain は無改修。Q4=冪等性を SQL の WHERE 句で作る・分類は報告用。Q5=cache/queue/CB/lock/index = N/A。

**生成物**: DP-U5-01〜04（読み取り経路の分割を関数名で強制 / 導出状態の保存化+単一アクセサ+原子性の相乗り / 冪等性の SQL 化 / 経路分離によるガードの保全）+ 非採用部品表。LC-U5-01〜07 + DataContract 追加（`ItemRetireRequest`/`RetireResult`）+ 呼び出し先の固定表 + 依存方向。

**次**: 標準 2 択（Request Changes / Continue → **Infrastructure Design〈U5〉**）。
