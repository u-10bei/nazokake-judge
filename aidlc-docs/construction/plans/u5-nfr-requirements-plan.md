...
- [x] `construction/u5/nfr-requirements/nfr-requirements.md`（U5-NFR-NN + 非目標）
- [x] `construction/u5/nfr-requirements/tech-stack-decisions.md`（TSD-U5-NN）
...
**Part 2 生成時の追加（質問になかった論点・要確認）**: **PBT-02（ラウンドトリップ）の適用可否**が質問に含まれていなかった。強制セット（PBT-02/03/07/08/09）ゆえ適用性の明示が必要。**U4b では「一方向変換ゆえ非該当」と明記したが、U5 は `sessions.likert_targets` の JSON シリアライズ/デシリアライズを新設するため該当**と判断し **U5-NFR-07** として追加（保存/復元が**順序を含めて一致**すること＝Likert の提示順が意味を持つため）。XC-02（セッション状態のラウンドトリップ・既存方針 = DB 行復元）の延長線上。→ 異論あれば Request Changes で。

**次**: 標準 2 択（Request Changes / Continue → **NFR Design〈U5〉**）。
