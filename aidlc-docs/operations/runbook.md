...
> | `manual-p-data.md` | **データ提供元** | **プール素材の提供仕様**（層の定義・`item_id`/本文の凍結規約・練習ペア） |
> | `smoke-prod.md` | P-RSCH 研究者 | **本番前スモーク**（使い捨てステージングで参加者フロー確認 → 削除） |
>
> **初めて本番に出す前に** → `dry-run-dev.md` でローカル一巡を通してから §1 のデプロイへ。
> **実インフラで参加者フローを確認したい**なら、§1 の前に `smoke-prod.md`（使い捨てステージング
> Worker + 別 D1 にデプロイ → ブラウザ確認 → 丸ごと削除。**本番 D1 を触らない**）。
...
- **層ラベル（必須）**: `pro`（プロ作品）/ `ai`（AI 生成）/ `edit`（編集・自作）/ `rule`（ルールベース生成）/ **`anchor`**（下帯アンカー・**役割層**）/ **`practice`**（練習専用・**充足の母数外**）。欠落・不正は投入拒否。
- **投入する件数**:
  - **U6（事前生成割当・実データ）**: **n=38**（`pro`10 / `anchor`2 / `edit`14 / `ai`9 / `rule`3）+ 練習素材。→ `plans/README.md`
  - **95 件（`pro`30 / `ai`20 / `edit`30 / `rule`15）はオンライン生成経路の較正構成**（要件 FR-08）。事前生成プランでは露出 gap が構成で 0 になるため**較正自体が不要**です。→ `manual-p-rsch.md` §4
- **⚠️ 参照済み item（= 誰かのペア列に出た item）への更新は拒否**されます（プール凍結 BR-U4a-03）。**ただしガードが効くのは参加開始後**——プラン確定後〜参加開始前は本文を差し替えられ、**プランもハッシュも `item_id` しか見ないため装置は検出できません**。提供元との合意事項は `manual-p-data.md` §4。
...
- 管理操作は `admin_log` が JSON で出力（`admin_auth_failed` / `pool_ingest` / `pool_sufficiency_warning` / `token_issue` / `token_issue_blocked` / `admin_progress` / `admin_winrates` / `admin_export` / `item_retire` / `item_unretire` / `plan_ingest` / `plan_activate` 等）。
- **⚠️ ログは永続化されません** — `wrangler tail` は**追っている間だけ**流れます。著作権対応の
  廃止履歴（`item_retire` の `item_ids`）やプラン投入の `content_hash` を証跡として残すなら、
  **操作時に tail を回して手元に保存**するか、Cloudflare Logpush など外部へ送る必要があります。
