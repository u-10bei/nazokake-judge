...
## 🚦 どちらの手順を辿るか（U6 以降・最初に読む）

**本書には 2 つの経路があります。実データでの確認は §3.5 が正です。**

| | §1〜§2（従来の一巡） | **§3.5（U6 カットオーバー）** |
|---|---|---|
| 前提 | プール 95 件・30 名・実行時にペアを生成 | **実データ n=38・E=8・事前生成プラン** |
| 用途 | 装置の一巡を通す**動作確認** | **実験本番と同じ手順のリハーサル** |
| トークン | `token_issue 30` | `token_issue 8`（**スロット数まで**） |

**⚠️ プランを有効化した状態で §2-② の `token_issue 30` を実行すると拒否されます**
（`count=30 が有効プランのスロット数 8 を超過`）。これは正常な動作です——トークン発行時に
`(plan_set, plan_index)` を束縛するため、**スロット数を超えて発行できません**。

**実データで確認したい場合は §0 →（§1 起動）→ §3.5** を辿ってください。§2 は
「U6 以前の挙動 / プラン未投入時のフォールバック経路」の確認用として残しています。

→ **プラン入力ファイルの準備**: `plans/README.md`（`composition.json` は記入済み・
`constraints.json` は雛形のまま＝**記入が必要**）
...
- **層ラベル必須**: `pro` / `ai` / `edit` / `rule` / **`anchor`**（下帯アンカー・役割層）/
  **`practice`**（練習専用・**充足の母数外**）。
- **U6 実データの構成は n=38**（`pro`10 / `anchor`2 / `edit`14 / `ai`9 / `rule`3）→ `plans/README.md`。
  95 件（`pro`30 / `ai`20 / `edit`30 / `rule`15）は**オンライン生成経路の較正構成**で、
  事前生成プランを使う U6 では**不要**です（露出 gap が構成で 0 になるため較正自体が要らない）。
- `body_ref` は任意（出自メモ）。
