...
#### U6: 層拡張（下帯アンカー）+ 事前生成割当（追加要件 2026-07-20）— ✅ **CLOSED**
...
- **Current Stage**: **CONSTRUCTION 完了 — 全 6 ユニット CLOSE**（U1〜U6）。次は **本番デプロイ**（運用者操作）
...
- **Next Stage**: **本番デプロイ**（Cloudflare 認証が要るため運用者側の操作）。migration 0005 を同時に載せる — ⚠️ **発行済み未消化トークンが無い時点に限る**（U6-NFR-04: データがある状態での親テーブル再構築ゆえ）。その後 **U2 ベータ最終 CLOSE**（本番 curl 疎通 3 点）→ 実験カットオーバー（`dry-run-dev.md` §3.5 で dev リハーサル済みの ⓪〜⑥ を本番で実施）
