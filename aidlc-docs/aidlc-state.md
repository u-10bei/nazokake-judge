...
- [x] Code Generation — **完了**（2026-07-20）。**全 19 Step 実装・4 ブロック分割生成**。**テスト結果: unit+PBT 108 緑 / integration 実 D1 52/52 PASS**（U6 15/15・回帰 U2 9・U3 8・U4a 7・U5 13）。**🔒 完了基準 4 点を実測確認**: `list_items()` 凍結（シグネチャ・SQL 無変更）/ `for layer in Layer` 走査の残存ゼロ / `save_pair_sequence` 以降無変更 / **Likert が固定リスト経路**。`assignment.py`・`likert.py`・`wrangler.toml`・`deploy.yml`・`frontend/`・`EXPORT_FORMAT_VERSION`(1.0.0) は無変更。**🔍 テストが実装の穴を 3 件検出**: ①**enum 走査がテスト基盤にも潜んでいた**（`generators.py` の `list(Layer)`・`test_pool_sufficiency` のオラクル・`test_likert_selection` の「4」ハードコード＝BR-U6-05 は実装だけを対象にしていた）②**ブロック連結の実行可能性検査が無かった**（PBT 反例 `n=10,m=4,E=3`＝ブロックが n−1 辺に届かない組合せは**どの seed でも不可能**なのに曖昧にリトライ失敗していた → `check_block_feasibility` を追加。**PU6-7 は「返されたプランがブロック連結」に述語を訂正**——U4b の単調性と同じ「性質の過剰主張」）③**★Likert 固定リストの件数が権威でなかった**（integration で発覚: 固定 3 件 + `likert_items=10` → **7 件がラウンドロビンで補充**＝FD Q2 で否決した挙動が部分的に復活 → プラン経路で `likert_items=len(fixed)` も渡し**補充ループに入らせない**。`likert.py` は無改修のまま）。文書: `construction/u6/code/README.md` / `dry-run-dev.md` §3.5 カットオーバー・リハーサル / `manual-p-rsch.md`（**anchor は役割層＝層順序予測に含めない**・**バーの定義は層平均でなく指名アンカーの β 位置**・**出現回数を共変量として検証**）
- [x] Code Generation 追補 — **`scripts/plan_ingest.py` 実装**（2026-07-20）。**レビューで欠落を検出**: Code Gen Q1=A で確定した「生成と投入の分離（CLI 2 本）」の**投入側が未実装**だった（`plan_generate/__main__.py` の docstring が存在しない `scripts/plan_ingest` を参照・README カットオーバー④が唯一コマンドの無いステップ）。API 側は完成していたため curl 手打ちで実験は成立するが、**Q1=A の要点だった「コミット済みファイルを読み内容ハッシュを再計算して照合してから POST」が手作業**になり、カットオーバーで最も事故らせたくない箇所に人的ミスの口が残っていた。**`content_hash` を `__main__.py` → `verify.py` へ移設**し `plan_ingest` が**同一実装を import**（再実装すると「自分の計算どうしが一致する」だけになり生成器とのずれを検出できない＝照合が自己満足になる。unit で `is` 同一性を固定）。**実機で 4 系統を検証**: 正常系（投入+activate）/ ★改竄検出（1 箇所書き換えで **POST せず** exit 1・未投入を確認）/ ファイル不在（次コマンド提示）/ ★409 ガード。**実測で 1 件是正**: 409 専用の案内が `--activate` 経路にしか無く、**運用上 409 に当たりやすい `--activate-only`** が汎用メッセージに落ちていた → `_activate()` に共通経路化。unit +9（**108 緑**）。文書: README ④・`dry-run-dev.md` ④ + 確認項目・**`runbook.md` §①-b 新設**（プランの記載が皆無だった）
- [ ] Build & Test — **承認待ち**（2026-07-20）。**19 Step**（これまでで最大: migration 0005 + 生成器 7 コンポーネント + API 2 本 + 引き当て置換 + 補充トークン + PBT 8 本）+ 決定点 6 問。決定点: CLI を生成/投入で分割（BR-U6-12 の「コミット」が間に挟まるため）/ LC 一対一のパッケージ分割 / 制約ファイルは JSON・`plan_set` 内包で誤適用防止 / **placement の目的関数は辞書式**（禁止=ハード → 層間はゲート到達で頭打ち → 濃縮最大化 → 忌避）/ プランは `plans/<set>/` にコミット（**gitignore 非該当を実測確認**）/ 回帰全緑ブロッキング
...
- **Current Stage**: **U6 層拡張+事前生成割当 — Build & Test 承認待ち**（standardized 2-option GATE）。実装・テスト・文書すべて完了
...
- **Next Stage**: U6 Build & Test 承認 → **U6 CLOSE = 全 6 ユニット完了**。その後 **本番デプロイ**（0005 を同時に載せる。⚠️ **発行済み未消化トークンが無い時点に限る** — U6-NFR-04）→ U2 ベータ最終 CLOSE（本番 curl 疎通 3 点）
